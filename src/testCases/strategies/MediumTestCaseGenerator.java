	...
	public LinkedList<BotiumTestCase> createTestCasesFromBranches(TreeBranch treeBranch, BotAnalyser botAnalyser) {
		...
		try {
			...
			while (treeBranch.hasNext()) {
				...
				// Split the training phrases considering the parameters of each phrase, and
				// categorise them in groups
				...
				// Estrategias aqui.
				// TODO: Aqui deberiamos poder trazar estrategias. Esta divide los grupos en
				// archivos.
				...
				for (ConversationGroup conversation : conversationGroupList) {
					...
					// It may have a single or multiple fragments per group: If the phrase is not
					// complete
					...
					if (!testcaseList.isEmpty()) {
						// Para cada uno de los test cases que hay, hay que añadirles los fragmentos
						// nuevos
						for (BotiumTestCase botTcIn : testcaseList) {
							...
							for (BotiumTestCaseFragment tcFragment : tcFragmentList) {
								...
								if (!isBinary(controlParameters[controlParameters.length - 1])) {
									...
									if (!flag) {
										...
										if (tcFragIntentName != null && isBinary(
												tcFragIntentName.split("_")[tcFragIntentName.split("_").length - 1])
												&& isFirst) {
											hasFragments = true;
	...
	private LinkedList<BotiumTestCaseFragment> complementConversation(ConversationGroup conversation,
		...
		if (!conversation.allParametersPresent()) {
			...
			for (Parameter param : paramList) {
				...
				System.out.println("Param: " + param.getName() + " | bot: " + defaultPrompts.getFirst() + " | user: "
						+ defaultResp.getFirst());

				// Create the fragment, and select the reverse mode
				// TODO: Actually, this is done by inline actions. It can be done by using these
				// lists (defaultResp and defaultPrompts)
				newFragment = new BotiumTestCaseFragment(defaultResp.getFirst(), defaultPrompts.getFirst(), true);
				// newFragment = new
				// BotiumTestCaseFragment(conversation.getIntentGroupName()+"_"+param.getName(),
				// conversation.getActionGroupName()+"_"+param.getName(), defaultResp,
				// defaultPrompts, true);
